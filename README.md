...
export PLUGIN_VERSION=v0.2.2
```

### build
#### Container (docker)
This executes all local build targets in a container:
* fetch the build information for the targeted Gloo Enterprise version
* Check if gloo and your plugin have different dependencies
* Build the plugin
```
make GLOOE_VERSION=$GLOOE_VERSION build
```
...
Run `resolve-deps` to check if gloo and your plugin have different dependencies:

```
make GLOOE_VERSION=$GLOOE_VERSION resolve-deps
```

If there are any mismatched dependencies, please use the information of outputted mismatched content to update your `go.mod` file. 
...
It is important to build the plugins on a linux operating system, because this os is compatible with Gloo Enterprise. 
...
This is caused by a dependency mismatch. Please run the `resolve-deps` target and update your `go.mod` file using the 
the information of outputted mismatched content.